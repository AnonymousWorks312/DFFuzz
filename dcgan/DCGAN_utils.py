...
class DCGAN:
    def __init__(self, dataset):
        if dataset == 'MNIST':
            ...
            self.model.load_weights('dcgan/discriminator_epoch100_mnist')
        elif dataset == 'CIFAR10':
            self.model = discriminator_model_cifar10()
            self.model.compile(loss='binary_crossentropy', optimizer="SGD")
            self.model.load_weights('dcgan/discriminator_epoch4_cifar10')
        elif dataset == 'FM':
            self.model = discriminator_model()
            self.model.compile(loss='binary_crossentropy', optimizer="SGD")
            self.model.load_weights('dcgan/discriminator_epoch100_fm')
        elif dataset == 'SVHN':
            self.model = discriminator_model_cifar10()
            self.model.compile(loss='binary_crossentropy', optimizer="SGD")
            self.model.load_weights('dcgan/discriminator_epoch16_svhn')
