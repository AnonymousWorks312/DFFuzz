# -*- coding: utf-8 -*-
import numpy as np
from utils.coverages.utils import get_layer_outs, get_layer_outs_new, calc_major_func_regions, percent_str, percent
...
class DeepGaugePercentCoverage(AbstractCoverage):
    def __init__(self, params, model, k=1000,train_inputs=None, major_func_regions=None, skip_layers=None, coverage_name="kmn",scaler=default_scale):
    ...
    # def initial_seed_list(self, test_inputs_all):
    #     # for test_inputs in test_inputs_all:
    #     ptr_section_by_section = np.tile(np.zeros(self.total_neuron_num*self.k, dtype=np.uint8), (len(test_inputs), 1))
    ...
    #                 sid = self.start + self.layer_start_index[layer_index] + neuron_index * self.bytearray_len + section_index
    ...
    #
    #             # self.activation_table_by_section = np.zeros(self.total_neuron_num * self.k, dtype=np.uint8)
    #             # self.nbc_table = np.zeros(self.total_neuron_num, dtype=np.uint8)
    #             # self.snac_table = np.zeros(self.total_neuron_num, dtype=np.uint8)
    #
    #             seed_id += 1
    #
    ...
    #
    #     # return (percent_str(multisection_activated, self.k * self.total_neuron_num),  # kmn
    #     #         multisection_activated,
    #     #         percent_str(nbc_activated, 2 * self.total_neuron_num),  # nbc
    #     #         percent_str(snac_activated, self.total_neuron_num),  # snac
    #     #         lower_activated, upper_activated, self.total_neuron_num,
    #     #         multisection_activated, upper_activated, lower_activated, self.total_neuron_num, outs)
        ...
        for layer_index, layer_out in enumerate(outs):  # layer_out is output of layer for all inputs
            ...
            for out_for_input in layer_out:  # out_for_input is output of layer for single input
                ...
                for neuron_index in range(out_for_input.shape[-1]):
                    ...
                    # self.neuron_set.add(global_neuron_index)
                    # print(self.major_func_regions[global_neuron_index])
                    ...
                    if section_index > self.k - 1:
                        sid = self.start + self.layer_start_index[layer_index] + neuron_index * self.bytearray_len +self.k - 1
                    else:
                        sid = self.start + self.layer_start_index[layer_index] + neuron_index * self.bytearray_len + section_index
                    # print(seed_id,sid)
                    ptr_section_by_section[seed_id][sid] = 1
    ...
    def get_current_coverage(self, with_implicit_reward=False):
        ...
        if self.coverage_name == "kmn":
            ...
            if nbc_activated == 0:
                return 0
            return percent(nbc_activated, 2 * total)  #  nbc
        elif self.coverage_name == "snac":
            if snac_activated == 0:
                return 0
            return percent(snac_activated, total)  # snac
        else:
            raise Exception("Unknown coverage: " + str(self.coverage_name))
...
def measure_k_multisection_cov(params,model, test_inputs, k, train_inputs=None, major_func_regions=None, skip=None, outs=None):
    if skip is None:
        skip = []
